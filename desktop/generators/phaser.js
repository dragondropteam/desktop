...
Blockly.JavaScript.addReservedWords(
    ...
    'break,case,catch,continue,debugger,default,delete,do,else,finally,for,function,if,in,instanceof,new,return,switch,this,throw,try,typeof,let,void,while,with,' +
...
Blockly.JavaScript.scrub_ = function(block, code) {
  ...
  if (!block.outputConnection || !block.outputConnection.targetConnection) {
    ...
    if (comment) {
      if (block.getProcedureDef || block.getMethodDef) {
    ...
    for (let i = 0; i < block.inputList.length; i++) {
      if (block.inputList[i].type == Blockly.INPUT_VALUE) {
        ...
        if (childBlock) {
          let comment = Blockly.JavaScript.allNestedComments(childBlock);
          if (comment) {
            commentCode += Blockly.JavaScript.prefixLines(comment, '// ');
          }
        }
 ...
 * @private
 */
Blockly.JavaScript.scrubClasses_ = function(block, code) {
    let commentCode = '';
    // Only collect comments for blocks that aren't inline.
    if (!block.outputConnection || !block.outputConnection.targetConnection) {
        // Collect comment for this block.
...
Blockly.JavaScript.getAdjusted = function(block, atId, opt_delta, opt_negate,
  ...
  let defaultAtIndex = Blockly.JavaScript.ONE_BASED_INDEXING ? '1' : '0';
  let at, innerOrder;
  if (delta > 0) {
     at = Blockly.JavaScript.valueToCode(block, atId,
     ...
     at = Blockly.JavaScript.valueToCode(block, atId, order) ||
        defaultAtIndex;
  }
  ...
  if (Blockly.isNumber(at)) {
    ...
    if (delta > 0) {
      ...
      at = at + ' - ' + -delta;
      innerOrder = Blockly.JavaScript.ORDER_SUBTRACTION;
    }
    if (opt_negate) {
      if (delta) {
        at = '-(' + at + ')';
      ...
      innerOrder = Blockly.JavaScript.ORDER_UNARY_NEGATION;
    }
