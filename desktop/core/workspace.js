...
/**
 * The following additions copyright DigiPen Institute of Technology 2016
 *
 */
Blockly.Workspace.prototype.changeVariableType = function(name, newType, cType){
  Blockly.Events.setGroup(true);
    let blocks = this.getAllBlocks();
  // Iterate through every block.
  for (var i = 0; i < blocks.length; i++) {
    blocks[i].retypeVar(name, newType, cType);
  }
  Blockly.Events.setGroup(false);
};

Blockly.Workspace.prototype.getVariableType = function(name) {
  Blockly.Events.setGroup(true);
  var blocks = this.getAllBlocks();
  // Iterate through every block.
  for (var i = 0; i < blocks.length; i++) {
    var field = blocks[i].getVariableFieldIfExists(name);
    if(field){
      return field.getCType();
    }
  }
  Blockly.Events.setGroup(false);
...
Blockly.Workspace.prototype.getVariableTypeExcludeId = function(name, id) {
  Blockly.Events.setGroup(true);
  var blocks = this.getAllBlocks();
  // Iterate through every block.
  for (var i = 0; i < blocks.length; i++) {
