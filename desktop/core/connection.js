...
/**
 * List of compatible value functionTypes.  Null if all functionTypes are compatible.
 * @type {Array}
 * @private
 */
...
/**
 * Whether this connection can accept promiscious connections. If true only typed connections will be allowed
 * @type {boolean}
 * @private
 */
Blockly.Connection.prototype.strict_ = false;
...
Blockly.Connection.prototype.checkConnection_ = function (target) {
    switch (this.canConnectWithReason_(target)) {
        ...
        case Blockly.Connection.REASON_WRONG_TYPE:
            throw 'Attempt to connect incompatible functionTypes.';
...
Blockly.Connection.prototype.isConnectionAllowed = function (candidate) {
    ...
    if (candidate.type == Blockly.INPUT_VALUE && candidate.isConnected() && !candidate.targetBlock().isMovable() && !candidate.targetBlock().isShadow()) {
        return false;
    }
    ...
    if (this.type == Blockly.PREVIOUS_STATEMENT &&
        candidate.isConnected() && !this.sourceBlock_.nextConnection && !candidate.targetBlock().isShadow() &&
...
Blockly.Connection.prototype.checkType_ = function (otherConnection) {
    if ((!this.check_ || !otherConnection.check_)) {
        ...
        return !(this.strict_ || otherConnection.strict_);
    }
 ...
 * @param {*} check Compatible value type or list of value functionTypes.
 *     Null if all functionTypes are compatible.
 * @param strict If untyped type connections are prohibited
 * @return {!Blockly.Connection} The connection being modified
 *     (to allow chaining).
 */
Blockly.Connection.prototype.setCheck = function (check, strict) {
    if (check) {
